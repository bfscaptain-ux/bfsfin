  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50 dark:bg-[#0b132b] text-slate-900 dark:text-slate-100 font-sans selection:bg-emerald-900 selection:text-white transition-colors duration-300">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
          ...
          >
            <span className="inline-flex items-center gap-2 text-xs uppercase font-extrabold text-emerald-700 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-500/10 px-4 py-1.5 rounded-full border border-emerald-200 dark:border-emerald-500/30 tracking-wider shadow-sm">
              ...
              <Building className="w-4 h-4" /> Core Product
            </span>
            <h1 className="text-3xl sm:text-4xl md:text-5xl font-black text-slate-900 dark:text-white leading-tight tracking-tight">
              Unlock Your <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-600 via-blue-600 to-amber-500 dark:from-emerald-400 dark:via-blue-400 dark:to-amber-400">Property's Value.</span>
            </h1>
            <p className="text-slate-600 dark:text-slate-300 text-sm sm:text-base max-w-xl leading-relaxed">
              Get high-value loans at lower interest rates by leveraging your residential or commercial property for business expansion or personal needs.
            </p>
          ...
          >
            ...
            <div className="space-y-3 text-sm">
                <div className="flex justify-between items-center py-2 border-b border-slate-100 dark:border-slate-800/60">
                  ...
                  <span className="font-bold text-emerald-700 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-500/10 px-3 py-1 rounded-md">9.50%</span>
                </div>
                <div className="flex justify-between items-center py-2 border-b border-slate-100 dark:border-slate-800/60">
                  <span className="text-slate-500 dark:text-slate-400 font-medium">Max Tenure</span>
                  <span className="font-bold text-slate-900 dark:text-white">15 Years</span>
                </div>
                <div className="flex justify-between items-center py-2">
                  <span className="text-slate-500 dark:text-slate-400 font-medium">LTV Ratio</span>
                  <span className="font-bold text-slate-900 dark:text-white">Up to 75%</span>
                </div>
            </div>
          </motion.div>
        </div>
      ...
      <main className="flex-1 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full space-y-24 py-16">
          ...
          <div className="flex justify-center mb-10">
            <div className="inline-flex bg-slate-100 dark:bg-slate-800 p-1 rounded-xl border border-slate-200 dark:border-slate-700">
              <button 
                ...
                className={`px-6 py-3 rounded-lg text-sm font-bold transition-all ${activeTab === 'residential' ? 'bg-white dark:bg-slate-700 text-blue-700 dark:text-blue-400 shadow-sm' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'}`}
              >
                Residential Property
              </button>
              <button 
                onClick={() => setActiveTab('commercial')}
                className={`px-6 py-3 rounded-lg text-sm font-bold transition-all ${activeTab === 'commercial' ? 'bg-white dark:bg-slate-700 text-blue-700 dark:text-blue-400 shadow-sm' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'}`}
              >
                Commercial Property
              </button>
            </div>
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-8 sm:p-10 rounded-3xl shadow-sm flex flex-col h-full hover:shadow-xl transition-shadow duration-300">
              ...
              <ul className="space-y-6 flex-1">
                <li className="flex items-start gap-4">
                  ...
                  <div>
                    <h4 className="font-bold text-slate-900 dark:text-white">Applicant Type</h4>
                    <p className="text-sm text-slate-600 dark:text-slate-400 mt-1">Salaried individuals, Self-Employed professionals, and Businesses.</p>
                  </div>
                </li>
                <li className="flex items-start gap-4">
                  <div className="w-10 h-10 bg-slate-50 dark:bg-slate-800 rounded-full flex items-center justify-center shrink-0 border border-slate-100 dark:border-slate-700">
                  ...
                  <div>
                    <h4 className="font-bold text-slate-900 dark:text-white">Property Value</h4>
                    {activeTab === 'residential' ? (
                      <p className="text-sm text-slate-600 dark:text-slate-400 mt-1">Minimum market value of ₹30 Lakhs.</p>
                    ) : (
                      <p className="text-sm text-slate-600 dark:text-slate-400 mt-1">Minimum market value of ₹50 Lakhs with clear titles.</p>
                    )}
                  </div>
                </li>
                <li className="flex items-start gap-4">
                  ...
                  <div>
                    <h4 className="font-bold text-slate-900 dark:text-white">Income Proof</h4>
                    <p className="text-sm text-slate-600 dark:text-slate-400 mt-1">Stable income to support EMI obligations.</p>
                  </div>
                </li>
              </ul>
            </div>
                ...
                <div>
                  ...
                  <ul className="space-y-2 text-sm text-slate-300">
                    <li className="flex items-center gap-3 bg-slate-800/50 dark:bg-slate-900/50 p-2.5 rounded-xl border border-slate-700/50">
                      <CheckCircle2 className="w-4 h-4 text-blue-400 shrink-0" /> PAN Card & Aadhaar Card of all co-applicants
                    </li>
                  </ul>
                </div>
                
                <div>
                  <h4 className="text-emerald-400 font-bold text-sm uppercase mb-3">2. Property Documents</h4>
                  <ul className="space-y-2 text-sm text-slate-300">
                    <li className="flex items-center gap-3 bg-slate-800/50 dark:bg-slate-900/50 p-2.5 rounded-xl border border-slate-700/50"><CheckCircle2 className="w-4 h-4 text-blue-400 shrink-0" /> Registered Sale Deed / Conveyance Deed</li>
                    <li className="flex items-center gap-3 bg-slate-800/50 dark:bg-slate-900/50 p-2.5 rounded-xl border border-slate-700/50"><CheckCircle2 className="w-4 h-4 text-blue-400 shrink-0" /> Latest Property Tax Receipt</li>
                    {activeTab === 'commercial' && (
                      <li className="flex items-center gap-3 bg-slate-800/50 dark:bg-slate-900/50 p-2.5 rounded-xl border border-slate-700/50"><CheckCircle2 className="w-4 h-4 text-blue-400 shrink-0" /> Approved Building Plan</li>
                    )}
                  </ul>
                </div>
              </div>
        ...
        <motion.section {...fadeIn} className="space-y-10">
          <div className="text-center max-w-3xl mx-auto">
            <h2 className="text-3xl sm:text-4xl font-black text-slate-900 dark:text-white">Why Choose BFS for LAP?</h2>
            <p className="mt-4 text-slate-600 dark:text-slate-400">Maximize the value of your property with higher LTVs and transparent processing.</p>
          </div>
          ...
          <motion.div variants={staggerContainer} initial="initial" whileInView="whileInView" viewport={{ once: true, margin: "-100px" }} className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <motion.div variants={fadeIn} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-8 rounded-3xl shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
              ...
              <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-3">High LTV Ratios</h3>
              <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed">Get up to 75% of your property's market value, ensuring you get the maximum possible funds.</p>
            </motion.div>
            ...
            <motion.div variants={fadeIn} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-8 rounded-3xl shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
              ...
              <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-3">Lower Interest Rates</h3>
              <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed">Secured loans offer significantly lower rates compared to standard business or personal loans.</p>
            </motion.div>
            ...
            <motion.div variants={fadeIn} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-8 rounded-3xl shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
              ...
              <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-3">Longer Tenures</h3>
              <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed">Repay comfortably with tenures extending up to 15 years for smaller EMIs.</p>
            </motion.div>
      ...
      <SEOContentBlock productName="Loan Against Property" />
