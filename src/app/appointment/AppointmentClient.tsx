  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-emerald-950 font-sans selection:bg-emerald-500/30">
      ...
      <main className="py-20 lg:py-32 relative overflow-hidden min-h-[90vh] flex flex-col justify-center">
        {/* Background Elements */}
        <div className="absolute inset-0 z-0 bg-white dark:bg-emerald-950">
          <div className="absolute top-0 right-0 w-[800px] h-[800px] bg-emerald-500/5 dark:bg-emerald-500/10 blur-[120px] rounded-full pointer-events-none transform -translate-y-1/2 translate-x-1/3" />
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
          ...
          {step < 4 && (
            <div className="text-center mb-12">
              <motion.div 
                ...
                className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-emerald-50 dark:bg-emerald-900/30 border border-emerald-200 dark:border-emerald-800 text-emerald-700 dark:text-emerald-400 text-xs font-bold tracking-widest uppercase mb-4"
              >
          ...
          {step < 4 && (
            <div className="mb-8 relative">
              ...
              <div 
                className="absolute top-1/2 left-0 h-1 bg-emerald-600 rounded-full transition-all duration-500 ease-in-out -translate-y-1/2"
              ...
              <div className="relative flex justify-between">
                {[1, 2, 3].map((i) => (
                  <div 
                    key={i}
                    className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm border-2 transition-colors duration-300 bg-white dark:bg-emerald-900
                      ${step >= i ? 'border-emerald-600 text-emerald-600 dark:text-emerald-400' : 'border-slate-200 dark:border-emerald-800 text-slate-400'}
                      ${step === i ? 'ring-4 ring-emerald-100 dark:ring-emerald-900/50' : ''}
                    `}
                  >
                    {i}
                  </div>
          ...
          {/* Main Card */}
          <div className="bg-white dark:bg-emerald-900 rounded-[2rem] border border-slate-200 dark:border-emerald-800 shadow-2xl shadow-slate-200/50 dark:shadow-none overflow-hidden relative min-h-[400px]">
            <AnimatePresence mode="wait">
                ...
                <motion.div key="step1" {...fadeInUp} className="p-8 md:p-12">
                      ...
                      <button
                        ...
                        className={`text-left p-6 rounded-2xl border-2 transition-all ${
                          consultationType === option.id 
                            ...
                            : 'border-slate-100 dark:border-emerald-800 hover:border-emerald-300 dark:hover:border-emerald-700'
                        }`}
                      >
                        <option.icon className={`w-8 h-8 mb-4 ${consultationType === option.id ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-400'}`} />
                  ...
                  <div className="mt-10 flex justify-end">
                    <button 
                      ...
                      className="px-8 py-3.5 bg-emerald-600 hover:bg-emerald-700 disabled:bg-slate-300 dark:disabled:bg-slate-800 text-white font-bold rounded-xl transition-all flex items-center gap-2"
                    >
              ...
              {step === 2 && (
                <motion.div key="step2" {...fadeInUp} className="p-8 md:p-12">
                            ...
                            <button
                              ...
                              className={`flex flex-col items-center justify-center p-3 rounded-xl border-2 transition-all ${
                                selectedDate === dateString
                                  ? 'border-emerald-600 bg-emerald-600 text-white shadow-lg shadow-blue-600/20'
                                  : 'border-slate-100 dark:border-emerald-800 bg-slate-50 dark:bg-emerald-900/50 hover:border-emerald-300 dark:hover:border-emerald-700 text-slate-600 dark:text-slate-400'
                              }`}
                            >
                              <span className={`text-xs uppercase font-bold mb-1 ${selectedDate === dateString ? 'text-emerald-100' : 'text-slate-400'}`}>{dayName}</span>
                              <span className={`text-xl font-black ${selectedDate === dateString ? 'text-white' : 'text-slate-900 dark:text-white'}`}>{dayNum}</span>
                              <span className={`text-xs ${selectedDate === dateString ? 'text-emerald-100' : 'text-slate-500'}`}>{monthName}</span>
                            </button>
                          );
                        })}
                      </div>
                    ...
                    <AnimatePresence>
                      {selectedDate && (
                        <motion.div 
                          ...
                          className="pt-4 border-t border-slate-100 dark:border-emerald-800"
                        >
                          <div className="flex items-center gap-2 text-sm font-bold text-slate-700 dark:text-slate-300 mb-4">
                              ...
                              <button
                                ...
                                className={`py-3 rounded-xl border-2 font-bold text-sm transition-all ${
                                  selectedTime === time
                                    ...
                                    : 'border-slate-100 dark:border-emerald-800 hover:border-emerald-300 dark:hover:border-emerald-700 text-slate-600 dark:text-slate-400'
                                }`}
                              >
                                {time}
                              </button>
                  ...
                  <div className="mt-10 flex justify-between items-center pt-6 border-t border-slate-100 dark:border-emerald-800">
                    <button 
                      onClick={handleBack}
              ...
              {step === 3 && (
                <motion.div key="step3" {...fadeInUp} className="p-8 md:p-12">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
                    {/* Summary Sidebar */}
                    <div className="md:col-span-5 bg-slate-50 dark:bg-slate-800/50 p-6 rounded-2xl border border-slate-100 dark:border-emerald-800">
                    ...
                    <form onSubmit={handleSubmit} className="md:col-span-7 space-y-5">
                      <div className="space-y-2">
                        ...
                        <input 
                          ...
                          className="w-full bg-slate-50 dark:bg-emerald-950 border border-slate-200 dark:border-emerald-800 text-slate-900 dark:text-white rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500"
                      ...
                      <div className="mt-8 flex justify-between items-center pt-6 border-t border-slate-100 dark:border-emerald-800">
                        <button 
                          type="button"
                          onClick={handleBack}
