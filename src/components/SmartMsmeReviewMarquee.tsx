...
import { Star, MapPin, ShieldCheck, ThumbsUp, X, ArrowRight, Play, Pause, ExternalLink, Sparkles, CheckCircle2, Building2, Award } from 'lucide-react';
...
interface SmartMsmeReviewMarqueeProps {
  reviews: Review[];
  totalReviewsCount?: number;
  loading?: boolean;
}

export default function SmartMsmeReviewMarquee({
  reviews,
  totalReviewsCount = 174,
  loading = false,
}: SmartMsmeReviewMarqueeProps) {
  const [isPaused, setIsPaused] = useState(false);
  const [isSlow, setIsSlow] = useState(false);
  const [selectedReview, setSelectedReview] = useState<Review | null>(null);
  const [likedReviews, setLikedReviews] = useState<Record<string, number>>({});

  // Split reviews into two lanes for dual-lane marquee
  ...
  // Extract MSME scheme tag from review text
  const getMsmeTag = (text: string) => {
    const t = text.toLowerCase();
    if (t.includes('cgtmse') || t.includes('collateral') || t.includes('bank loan') || t.includes('sanction')) {
      return 'CGTMSE Collateral-Free Loan';
    }
    if (t.includes('pmegp') || t.includes('subsidy') || t.includes('mudra')) {
      return 'PMEGP Subsidy / Mudra Scheme';
    }
    if (t.includes('gem') || t.includes('tender') || t.includes('samadhaan') || t.includes('delayed payment')) {
      return 'GeM Portal & MSME Samadhaan';
    }
    if (t.includes('machinery') || t.includes('working capital') || t.includes('factory')) {
      return 'Machinery & Working Capital CC';
    }
    return 'Udyam Registration (Lifetime)';
  };
  ...
  return (
    <div className="relative w-full overflow-hidden py-4 select-none">
      ...
      <style jsx global>{`
        @keyframes msme-marquee-left {
          0% { transform: translateX(0%); }
          100% { transform: translateX(-50%); }
        }
        @keyframes msme-marquee-right {
        ...
        .msme-track-left {
          display: flex;
          width: max-content;
          animation: msme-marquee-left var(--msme-speed, 50s) linear infinite;
        }
        .msme-track-right {
          display: flex;
          width: max-content;
          animation: msme-marquee-right var(--msme-speed, 55s) linear infinite;
        }
        .msme-track-left:hover,
        .msme-track-right:hover {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-6 flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-bold text-slate-700 dark:text-emerald-300">
            Live Stream: {totalReviewsCount}+ Verified MSME Enterprises
          </span>
        </div>
      ...
      <div 
        ...
        style={{ '--msme-speed': isSlow ? '90s' : '45s' } as React.CSSProperties}
      >
        {/* Left and Right Smooth Fade Gradients */}
        <div className="pointer-events-none absolute inset-y-0 left-0 w-16 sm:w-32 bg-gradient-to-r from-white dark:from-[#031814] to-transparent z-20" />
        <div className="pointer-events-none absolute inset-y-0 right-0 w-16 sm:w-32 bg-gradient-to-l from-white dark:from-[#031814] to-transparent z-20" />

        {/* Row 1: Leftward Scrolling Track */}
        <div className={`msme-track-left gap-5 mb-5 ${isPaused ? 'msme-paused' : ''}`}>
          {[...lane1, ...lane1, ...lane1].map((rev, idx) => (
            <div
              key={`row1-${rev.id}-${idx}`}
              onClick={() => setSelectedReview(rev)}
              className="w-[330px] sm:w-[380px] shrink-0 p-5 rounded-2xl bg-slate-50 dark:bg-[#07271f] border border-slate-200/90 dark:border-emerald-700/40 shadow-sm hover:shadow-xl hover:border-emerald-500/80 transition-all duration-300 cursor-pointer flex flex-col justify-between relative overflow-hidden group/card hover:-translate-y-1"
            >
              ...
              <div className="pt-3 border-t border-slate-200/80 dark:border-emerald-800/60 flex items-center justify-between relative z-10 mt-auto">
        ...
        <div className={`msme-track-right gap-5 ${isPaused ? 'msme-paused' : ''}`}>
      ...
      {/* Interactive MSME Case Spotlight Modal */}
      {selectedReview && (
          ...
          <div className="relative bg-white dark:bg-[#07251d] w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden border border-emerald-500/30 animate-in zoom-in-95 duration-200 z-10">
            ...
            <div className="bg-gradient-to-r from-emerald-900 via-teal-900 to-emerald-950 text-white p-6 relative">
              ...
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-[11px] uppercase tracking-wider font-extrabold text-emerald-200 bg-emerald-950/60 px-2 py-0.5 rounded-full">
                  Verified MSME Enterprise Record
                </span>
              </div>
            ...
            <div className="p-6 space-y-5">
              ...
              <div className="grid grid-cols-2 gap-3 bg-slate-50 dark:bg-emerald-950/50 p-3 rounded-xl border border-slate-100 dark:border-emerald-800/60 text-xs">
                <div>
                  <span className="text-slate-400 dark:text-emerald-400/80 block text-[10px] uppercase font-bold">Government Scheme</span>
                  <span className="font-bold text-slate-800 dark:text-white">{getMsmeTag(selectedReview.text)}</span>
                </div>
                <div>
                  <span className="text-slate-400 dark:text-emerald-400/80 block text-[10px] uppercase font-bold">Registration Validity</span>
                  <span className="font-bold text-emerald-600 dark:text-emerald-400 flex items-center gap-1">
                    <CheckCircle2 className="w-3.5 h-3.5" />
                    Lifetime Government Validity
                  </span>
                </div>
              </div>
              ...
              {/* Action Button: Jump to MSME form */}
              <button
                onClick={() => {
              ...
              >
                <span>Register Your MSME Like {selectedReview.name.split(' ')[0]} with BFS</span>
